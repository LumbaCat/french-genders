...
function commencer(){
	if (marche == 0){
		...
		// ajout "mot"
		var mot = sections[section][k].ru;
		document.getElementById("mot").innerHTML = mot;

		radioboxes(section, k);
...
function rootCommencer(){
	...
	p.innerHTML  = "<i> Selectionnez le bon genre et changez de section après 20 bonnes réponses.</i>";
...
function radioboxes(section, k){
	...
	*/
		...
		radios[0].setAttribute("value", 0);
		radios[1].setAttribute("value", 1);
...
function verifier(id) {
		...
		var motTrad = sections[section][k].trad;

		tour++;
		document.getElementById('tour').innerHTML = tour;
		document.getElementById('sol').innerHTML = `${motFr} ${motRu} (${motTrad})`;
		if((motFr == "un")&&(valeur == 0) || (motFr == "une")&&(valeur == 1)){
...
function voirMots(){
	if(voir%2 == 0){
		...
		var tableauTrad = new Array(sections[section].length+1);

		tableauFr[0]="Genre";
		tableauRu[0]="Mot";
		tableauTrad[0]="Traduction";

		for(var i=1; i<sections[section].length+1; i++){
			tableauFr[i]=sections[section][i-1].fr;
			tableauRu[i]=sections[section][i-1].ru;
			tableauTrad[i]=sections[section][i-1].trad;
		}
	...
	function construireTab() {
		if (i == 0) {	
			...
			td3.innerHTML = tableauTrad[i];
			td3.setAttribute("class", "russeTitre");	
		}
		else {
			td1.innerHTML = tableauFr[i];
